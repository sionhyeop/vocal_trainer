...
// 구조화출력(json_schema)은 제약 디코딩이라 느리고 콜드 첫 호출에 스키마 1회 컴파일(~25s 스파이크)이
// 붙는다. 속도 위해 제거하고, 프롬프트로 JSON-only를 지시한 뒤 응답 텍스트에서 첫 JSON 객체만 안전 추출한다.
...
- 개선점·연습팁은 각 2개씩, 모든 항목은 1문장으로 간결하게.
- 반드시 아래 JSON 객체 하나만 출력. 마크다운/코드펜스/다른 설명 문장 절대 금지:
{"강점":"…","개선점":["…","…"],"연습팁":["…","…"],"한줄응원":"…"}`
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // JSON-only 지시했지만 혹시 잡설/코드펜스가 섞여도 첫 JSON 객체만 추출.
    const coaching = extractJson(textBlock.text)
